...
class MyButton {
  public:
    ...
    bool holdTimeOnce(int num) {
      ...
      if (currentlyPressed && !holdTriggered) {
        if (num == 0) {
          holdTriggered = true;
          return true;
        }
        if (pressStartTime != 0 && millis() - pressStartTime >= threshold) {
          holdTriggered = true;
          return true;
        }
      }
...
class MyTrack {
  public:
        ...
        stepper(AccelStepper::DRIVER, stepPin, dirPin)
    {
      ...
      findBoundsState = 0;
      endBoundTriggered = false;
      latchedTrackLength = 0;
    }
  ...
  private:
    ...
    bool endBoundTriggered;
    long latchedTrackLength;
    ...
    void handleLimitSwitches() {
      if (resetting) {
        ...
        if (findBoundsState == 1) {
          ...
          if (!endBoundTriggered && (endLS.isPressed() || endLS.isCurrentlyPressed())) {
            latchedTrackLength = stepper.currentPosition();
            stepper.stop();
            endBoundTriggered = true;
          }

          if (endBoundTriggered && !stepper.isRunning()) {
            tracklength = latchedTrackLength;
            stepSize = tracklength / subdivs;
            stepper.setCurrentPosition(tracklength);
