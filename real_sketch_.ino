...
//CUSTOM CLASSES------------------------------------------------------------------------
  ...
  class MyLED {
    public:
      // Constructor: sets the pin as OUTPUT and initialises the LED to OFF
      MyLED(int pinNum) {
        pin = pinNum;
        pinMode(pin, OUTPUT);
        state = false;          // LED starts off
      ...
      // Toggles the LED: if on, turn off; if off, turn on
      void toggle() {
        if (state) {
          turnOff();
        } else {
    ...
    private:
      int pin;
      bool state;   // remembers current state (true = ON, false = OFF)
  };

  class MyLightstrip {
    public:
      // Constructor: sets up the LED strip
      // Note: DATA_PIN is defined as A7 at the top of the code
      MyLightstrip(int dataPin, int ledNum) {
        numLeds = ledNum;
        leds = new CRGB[numLeds];          // allocate array for LEDs
        
        // FIXED: Use the actual pin directly since DATA_PIN is a constant
        // The dataPin parameter is kept for compatibility but we use the #define
        ...
        // Initialise pattern state
        CurrPattern = 0;
        
        // Initialize all state variables to prevent undefined behavior
        lastUpdate = 0;
        patternIndex = 0;
        hue = 0;
        movingIndex = 0;
      ...
      // Displays the current pattern; returns the pattern number
      int play() {
        unsigned long now = millis();

        switch (CurrPattern) {
          case 0:  // Standby: fill with cyan one LED at a time
          ...
          case 1:  // Game idle: smooth colour cycle over whole strip
            if (now - lastUpdate >= 20) {   // 20ms steps for smooth transition
                lastUpdate = now;
                hue += 1;                    // increment hue
          ...
          case 2:  // Track moving: single multi‑coloured pixel moves
            if (now - lastUpdate >= 100) {   // move every 100ms
                lastUpdate = now;
                // Clear strip
                FastLED.clear();
                // Light the current pixel with a hue that changes over time
                leds[movingIndex] = CHSV(movingHue, 255, 255);
                movingHue += 32;               // change colour for next move
          ...
          case 3:  // Player 1 wins: green blink
          ...
          case 4:  // Player 2 wins: blue blink
            if (now - lastUpdate >= 500) {
              lastUpdate = now;
              blinkState = !blinkState;
              if (blinkState) {
          ...
          case 5:  // Track resetting: red breathing (brightness varies)
            // Use a sine wave to modulate brightness (0‑255)
            // We'll update every 30ms for smooth breathing
            if (now - lastUpdate >= 30) {
              lastUpdate = now;
              // Increase angle, wrap at 2π (256 steps)
              breathAngle = (breathAngle + 4) % 256;  // 4/256 ≈ 5.6° per step
              // Get sine value (0-255) directly - no need to scale
              uint8_t brightness = sin8(breathAngle); // sin8 returns 0-255 already
              // Set all LEDs to red with that brightness
      ...
      // Changes the current pattern if num is valid (0‑5)
      bool changePattern(int num) {
          if (num >= 0 && num <= 5) {
              if (num != CurrPattern) {
                  CurrPattern = num;
                  resetPatternState();   // initialise state for the new pattern
              }
              return true;
          }
          return false;
      ...
      void turnOff() {
        FastLED.clear();
        FastLED.show();
        // Also reset pattern state to prevent unexpected behavior when turning back on
        resetPatternState();
      }

    private:
      int numLeds;
      CRGB* leds;                // dynamic array for LEDs
      int CurrPattern;            // current pattern index (0‑5)

      // Timing
      unsigned long lastUpdate;   // last time pattern was updated

      // Pattern‑specific state variables (reused, reset on pattern change)
      int patternIndex;           // for pattern 0: how many LEDs are on
      uint8_t hue;                // for pattern 1: current hue (0‑255)
      int movingIndex;            // for pattern 2: current LED position
      uint8_t movingHue;          // for pattern 2: colour of moving pixel
      bool blinkState;            // for patterns 3 & 4: whether LEDs are on
      uint8_t breathAngle;        // for pattern 5: angle in sine wave (0‑255)

      // Resets all pattern‑specific variables to sensible defaults for the current pattern
      void resetPatternState() {
        // Clear the LED array visually (optional, but good practice)
        FastLED.clear();
        FastLED.show();
        ...
        // Reset all shared state variables
        patternIndex = 0;
        hue = 0;
        movingIndex = 0;
        movingHue = 0;
        ...
        // Pattern‑specific initialisations
        switch (CurrPattern) {
          case 0:
            // patternIndex already 0
            break;
          case 1:
            // hue already 0
            break;
          ...
          case 3:
          case 4:
            blinkState = false;   // start with LEDs off
            break;
          case 5:
            breathAngle = 0;
            break;
...
//CLASS OBJECTS-------------------------------------------------------------------------
  ...
  // Status LED (on when game is on)
  MyLED statusLED(START_LED);

  // Additional status LEDs for future use
  MyLED led1(LED1);
  MyLED led2(LED2);

  // LED strip (adjust number of LEDs as needed, e.g., 30)
...
//CUSTOM FUNCTIONS----------------------------------------------------------------------
  // Update LED1 and LED2 according to currentMode
  void updateModeLEDs() {
    switch (currentMode) {
      case 0:
        led1.turnOff();
  ...
  void setGameOn(bool on) {
    ...
    if (on) {
      ...
      // Set mode LEDs according to current mode
      updateModeLEDs();

      // Enable motors? They will be enabled when we call move commands.
      // For safety, we leave them disabled until a move is requested.
      // You may optionally enable them here, but tracks handle enabling internally.
    } else {
      // Turn off status LED
      statusLED.turnOff();
      ...
      // Stop all tracks and disable their drivers
...
//MODE FUNCTIONS------------------------------------------------------------------------
  ...
  void runMode3() {
    // SINGLE PLAYER HARD MODE - Track moves forward on press, slowly drifts back
    
    // Mode-specific state variables (static so they persist between loop calls)
    static bool gameWon = false;           // True if player reached the end
    static bool waitingForReset = false;    // True after win, waiting for button
    static bool isResetting = false;        // True while track is resetting after win
    static bool isMovingForward = false;     // True during forward move (on press)
    static bool isDrifting = false;         // True during slow backward drift
    
    static unsigned long lastDriftTime = 0;  // Timer for drift movement
    const int DRIFT_SPEED = 200;             // Steps per second (adjust as needed)
    const int DRIFT_INTERVAL = 500;          // Check every 500ms
    ...
    if (modeJustChanged) {
        ...
        // Ensure track is at start position
        if (!track1.isAtStart()) {
            track1.resetTrack();
        }
        
        Serial.println("Mode 3: Hard Mode - Track slowly drifts back! Keep pressing!");
    }
    
    // --- WIN STATE: Player has reached the end ---
    if (gameWon) {
        // Win pattern is pattern 1 (idle/win) - FIXED: using getter
        if (lightStrip.getCurrentPattern() != 1) {
            lightStrip.changePattern(1);  // Pattern 1 = idle/win
            Serial.println("PLAYER WINS! Press button to play again");
        }
    ...
    // --- FORWARD MOVEMENT (on press) ---
    static bool lastPlateState = false;
    
    if (platePressed && !lastPlateState && !gameWon) {
        // Player just pressed the plate - trigger a forward move
        if (!track1.isAtEnd()) {  // Only move if not at end
            track1.moveNextPosition();
            isMovingForward = true;
            isDrifting = false;  // Cancel any ongoing drift
            Serial.println("Forward move");
        }
    }
    lastPlateState = platePressed;
    
    // Check if forward movement completed
    if (isMovingForward && !track1.isMoving()) {
        isMovingForward = false;
        ...
        // Start drifting backward (but only if not at start!)
        if (!track1.isAtStart()) {
            isDrifting = true;
    ...
    // --- SLOW BACKWARD DRIFT ---
    if (isDrifting && !gameWon) {
        // IMPORTANT: Stop drifting if we've reached the start
        if (track1.isAtStart()) {
            isDrifting = false;
            Serial.println("At start - drift stopped");
        }
        else if (millis() - lastDriftTime >= DRIFT_INTERVAL) {
            lastDriftTime = millis();
            
            // Calculate how many steps to move back
            // DRIFT_SPEED steps per second * (DRIFT_INTERVAL/1000) seconds
            int driftSteps = (DRIFT_SPEED * DRIFT_INTERVAL) / 1000;
            if (driftSteps < 1) driftSteps = 1;  // Minimum 1 step
            
            // Use the driftBack method instead of direct stepper access
            if (!track1.isMoving()) {
                track1.driftBack(driftSteps, DRIFT_SPEED);
                Serial.print("Drifting back ");
                Serial.print(driftSteps);
                Serial.println(" steps");
            }
        }
    }
    ...
    // --- LED STRIP PATTERN LOGIC ---
    if (isMovingForward) {
        // Moving forward - show pattern 2 - FIXED: using getter
        if (lightStrip.getCurrentPattern() != 2) {
            lightStrip.changePattern(2);  // Pattern 2 = moving pixel
        }
    } else {
        // Idle or drifting - show pattern 1 - FIXED: using getter
        if (lightStrip.getCurrentPattern() != 1) {
            lightStrip.changePattern(1);  // Pattern 1 = idle
        }
    }
